import json
import os
from pslist import get_new_pslist
from static_config import DLLS_IN_SYSDIR
from Utils.util import dump_process, dump_dll
from vol_command import execute_volatility_command
from Config.vol_path import EXPLOITABLE_PROCESS_NAMES, NORMAL_PROCESS_NAMES
from pefile import PEFormatError
from pe_utils import static_analysis, get_strings, fix_pe_from_memory
import pefile
        ...
        if process['Name'].lower() in EXPLOITABLE_PROCESS_NAMES:
            ...
            for child_process in pslist:
                ...
                if child_process['PPID'] == process['PID']:
                    if (child_process['Name'] != process['Name'] or child_process['Name'] != 'explorer.exe'):
                        ...
                        if dump_objects:
                            ...
                            dump_process(memory_instance, child_process['PID'], workdir,
                                         ...
                                         memdump=True)
    for proc in pslist:
        if proc['Name'].lower() not in NORMAL_PROCESS_NAMES:
            suspect_processes.append(proc['Name'])
...
def heuristic_injected_code(memory_instance, pslist=None, workdir=None, dump_objects=False, delete_non_pe=False):
    ...
    if dump_objects:
        logging.info('dumping injected processes to {}'.format(workdir))
        ...
        for single_dump in os.scandir(workdir):
            ...
            if len(splitted_line) == 6:
                ...
                try:
                    ...
                    outputpath = os.path.join(workdir, procname+ '.' + imagebase + '.fixed_bin')
        ...
        for suspect_path in suspect_path_list:
            if '\\'.join(loaded_dll['Path'].lower().split('\\')[0:-1]) == suspect_path.lower():
                if loaded_dll['Path'].lower() not in statistic_anomalies_list:
                    ...
                    if dump_objects:
                        logging.info(' dumping {} due to suspicious path'.format(loaded_dll))
...
def heuristic_dest_port_anomallies(memory_instance, pslist=None, workdir=None, dump_objects=False):
    ...
    suspicious_processes = list()
    WINXP = bool(re.match(r'^WinXP*', profile))
    WIN2003 = bool(re.match(r'^Win2003*', profile))
    if (WINXP == True or WIN2003 == True):
        connections = execute_volatility_command(memory_instance, 'connections')
        if len(connections) == 0:
            connections = execute_volatility_command(memory_instance,'connscan')
        ...
        connections = execute_volatility_command(memory_instance, 'netscan')

        for conn in connections:
            dst_ip, dst_port = conn['ForeignAddr'].split(':')
            if dst_port.isdigit():
                if dst_port not in whitelisted_dest_ports:
                    suspicious_processes.append(conn)
                    if dump_objects:
...
def heuristic_ssdt(memory_instance, pslist=None, workdir=None, dump_objects=False):
    ...
    for driver_name in known_owners:
        #vol.py --profile=WinXPSP2x86 -f "/home/MemoryDumps/APT.img" moddump -r irykmmww.sys -D /tmp
